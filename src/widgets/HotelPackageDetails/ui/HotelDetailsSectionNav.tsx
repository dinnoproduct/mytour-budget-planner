...
export const HotelDetailsSectionNav = ({
  ...
  const [activeId, setActiveId] = useState<HotelDetailsSectionId>(
    HOTEL_DETAILS_SECTION_IDS.overview,
  );
  ...
  const pickActiveSectionId = useCallback((): HotelDetailsSectionId => {
    const navEl = navMeasureRef.current;
    const markerY = navEl
      ? navEl.getBoundingClientRect().bottom + 4
      : TOP_OFFSET_PX + navHeight + 4;

    let current: HotelDetailsSectionId = NAV_ITEMS[0].id;
  ...
  const scrollToSection = (id: HotelDetailsSectionId) => {
    setActiveId(id);
      ...
      {NAV_ITEMS.map(({ id, labelKey }) => {
        ...
        return (
          <Button
            ...
            px={{ base: 3, md: 4 }}
            whiteSpace="nowrap"
            variant={isActive ? "solid-blue" : "solid-gray"}
