...
import { CyprusSearchForm } from '@widgets/PackageSearch/ui/CyprusSearchForm'
import {
  CyprusTabItem,
  HotelTabItem,
  GroupTabItem,
...
import { packageSearchGradients } from '@widgets/PackageSearch/ui/theme'

export const CyprusSearchMenu = ({
  onTabChange,
  onFormOpen,
  onFormClose,
  isFormOpen,
  ...
  const cityLabel = useMemo(() => {
    return cities
      .filter((c) =>
        Array.isArray(searchData.selectedCity)
          ? searchData.selectedCity.includes(c.id)
          : [searchData.selectedCity].includes(c.id),
      )
      .map(
        (c) =>
    ...
    <Box height="full" width="full">
      ...
      <Box width="full">
        <Box display={isFormOpen ? 'block' : 'none'}>
          <Flex justify="flex-end" align="center" width="full" p="3">
          ...
          >
            <CyprusSearchForm onSearch={handleFormClose} />
          </VStack>
        </Box>
